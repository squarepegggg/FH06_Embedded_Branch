...
void run_nn_infer(struct bma400_fifo_sensor_data *accel_data, uint16_t count)
{
	...
	/* Map EI v5 labels to index 0-4 */
	if      (strcmp(label, "class 1") == 0) biggest_idx = 0; /* clap      */
	else if (strcmp(label, "class 2") == 0) biggest_idx = 1; /* idle      */
	else if (strcmp(label, "class 3") == 0) biggest_idx = 2; /* roll      */
	else if (strcmp(label, "class 4") == 0) biggest_idx = 3; /* six_seven */
	else if (strcmp(label, "class 5") == 0) biggest_idx = 4; /* wave      */
	else biggest_idx = 0;
}
